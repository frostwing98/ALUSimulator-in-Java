...
public class ALU {
	...
	private static char XOR(char operand1,char operand2){
	...
	public static char calcCarry(char operand1,char operand2,char carry){
	...
	public static String integerRepresentation (String number, int length) {
		int digitinput=Integer.parseInt(number);
		...
		for(int i=0;i<length;i++){
			if(digit!=0){
				temp[i]=(char)((digit%2)+'0');
				digit=digit/2;
			}
			else
				temp[i]='0';
		}
		//mod 2,generating reversed binary string of abs(input)
		
		for(int k=0;k<length;k++)
				swap[length-1-k]=temp[k];
		String outputstr=new String(swap);
		if(!ispositive){
			output=negation(outputstr);//reverse back to normal,and if negative, ~(input)
			output=oneAdder(output);
	...
	public static String negation (String operand) {
		...
		String result=new String(temp);// TODO YOUR CODE HERE.
		return result;
	}
